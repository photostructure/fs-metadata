...
private:
    ...
    static DWORD WINAPI WorkerThread(LPVOID param) {
        auto* data = static_cast<std::pair<ThreadData*, std::shared_ptr<WorkQueue>>*>(param);
        ...
        while (threadData->running && !queue->IsShutdown()) {
            ...
            if (queue->Pop(task, 1000)) {
                try {
                    ...
                    DEBUG_LOG("[ThreadPool] Worker thread %lu caught exception", threadData->id);
                }
            }
        }
...
public:
    explicit ThreadPool(size_t numThreads = 4) : queue(std::make_shared<WorkQueue>()) {
        ...
        for (size_t i = 0; i < numThreads; ++i) {
            ...
            threadData->handle = CreateThread(
                nullptr, 0, WorkerThread, param, 0, &threadData->id);
