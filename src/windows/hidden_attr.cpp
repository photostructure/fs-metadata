...
public:
    SetHiddenWorker(Napi::Env env, std::string p, bool v, Napi::Promise::Deferred def)
        : Napi::AsyncWorker(env), 
          path(std::move(p)),
          value(v),
          deferred(std::move(def)) {}
...
Napi::Promise GetHiddenAttribute(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        auto* worker = new GetHiddenWorker(env, std::move(path), std::move(deferred));
        worker->Queue();
...
Napi::Promise SetHiddenAttribute(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        auto* worker = new SetHiddenWorker(env, std::move(path), value, std::move(deferred));
        worker->Queue();
