...
public:
    ...
    static bool IsPathSecure(const std::string& path) {
        ...
        for (const auto& device : deviceNames) {
            ...
            if (upperPath.find("\\" + device) != std::string::npos ||
                ...
                // Check if it's followed by a backslash, forward slash, dot, or end of string
        ...
        if (path.size() >= 4) {
            if ((path.substr(0, 4) == "\\\\?\\") || (path.substr(0, 4) == "\\\\.\\")) {
    ...
    static bool HasRequiredPrivileges() {
        ...
        BOOL result = GetTokenInformation(token, TokenElevation, 
    ...
    static std::wstring SafeStringToWide(const std::string& str, size_t maxLength = MAX_PATH) {
        if (str.empty()) {
            return L"";
        }
        ...
        if (!MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
                                str.c_str(), -1, &result[0], requiredSize)) {
...
public:
    ...
    public:
        explicit Lock(CriticalSectionGuard& g) : guard(g) {
            guard.Enter();
        }
