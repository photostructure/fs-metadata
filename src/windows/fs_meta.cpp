...
namespace FSMeta
{
    ...
    void Execute() override
    {
      try
      {
        ...
        if (GetLogicalDriveStringsA(length, driveStrings.data()) == 0)
        {
          DWORD error = GetLastError();
          char msg[256];
          sprintf_s(msg, "GetLogicalDriveStrings data failed with error: %lu", error);
          throw std::runtime_error(msg);
        }

        for (const char *drive = driveStrings.data(); *drive; drive += strlen(drive) + 1)
        {
          OutputDebugStringA(("Found local drive: " + std::string(drive) + "\n").c_str());
  ...
  public:
    ...
    void Execute() override
    {
      try
      {
                ...
                sizeof(fileSystem)))
        {
          DWORD error = GetLastError();
          char msg[256];
          sprintf_s(msg, "GetVolumeInformation failed with error: %lu for path: %s",
        ...
        if (!GetDiskFreeSpaceExA(mountPoint.c_str(),
                                 &freeBytes,
                                 &totalBytes,
                                 &totalFreeBytes))
        {
    ...
    void OnOK() override
    {
      ...
      result.Set("label", metadata.label.empty() ? Env().Null() : Napi::String::New(Env(), metadata.label));
      result.Set("fileSystem", metadata.fileSystem.empty() ? Env().Null() : Napi::String::New(Env(), metadata.fileSystem));
      ...
      result.Set("uuid", metadata.uuid.empty() ? Env().Null() : Napi::String::New(Env(), metadata.uuid));
