...
const color = (colorCode, text) => isWindows ? text : `${colorCode}${text}${colors.RESET}`;
...
if (process.env.ENABLE_ASAN) {
  if (os.platform() === 'linux') {
    ...
    try {
      ...
      try {
        const asanLib = execSync('clang -print-file-name=libclang_rt.asan-x86_64.so', { encoding: 'utf8' }).trim();
      ...
      console.log(color(colors.YELLOW, 'Running tests with AddressSanitizer...'));
      
      // Capture ASAN output for analysis
      let asanOutput = '';
      try {
        asanOutput = execSync('npm test -- --no-coverage 2>&1', { env }).toString();
      ...
      fs.writeFileSync(outputFile, asanOutput);
      ...
      const hasOurErrors = lines.some(line => 
        (line.includes('ERROR: AddressSanitizer') || line.includes('ERROR: LeakSanitizer')) &&
        (line.includes('fs_metadata.node') || line.includes('/src/'))
      );
      
      const hasOurLeaks = lines.some(line =>
        (line.includes('Direct leak') || line.includes('Indirect leak')) &&
        lines.slice(Math.max(0, lines.indexOf(line) - 5), lines.indexOf(line) + 10).some(context =>
          context.includes('fs_metadata.node') || context.includes('/src/')
        )
      );
      
      // Count V8/Node internal leaks for information
      const internalLeaks = (asanOutput.match(/leak.*\/usr\/bin\/node/g) || []).length;
      ...
      if (hasOurErrors || hasOurLeaks) {
        ...
        const relevantErrors = lines.filter((line, idx) => {
          if (line.includes('ERROR:') || line.includes('leak')) {
            ...
            return context.includes('fs_metadata.node') || context.includes('/src/');
          }
          return false;
        });
        ...
        console.log(color(colors.GREEN, '✓ AddressSanitizer tests passed (no issues in fs-metadata code)'));
        if (internalLeaks > 0) {
          console.log(color(colors.YELLOW, `   Note: ${internalLeaks} V8/Node.js internal leaks detected (suppressed)`));
        }
      }
    } catch (error) {
      if (error.code === 1) {
        console.log(color(colors.YELLOW, 'clang not found. Skipping AddressSanitizer tests.'));
      } else {
